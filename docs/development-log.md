...
## 3. Current state (as of 2026-05-14)
...
| **Registries** | 60 ECC agents + 323 skills + 75 commands + 1 MCP + 105 lang-rules + 3 hook-profiles + 3 built-ins + 34 native records. Harness plugin counts are machine-specific and refreshed from the local operator cache. Byte-stable across rebuilds for the portable sources. |
| **Tests** | 120 unit + 2 hook + 1 hook-route Node integration + 1 bootstrap-idempotency + 8 with-profile integration tests. 0 failures (excluding audit-findings-status pre-iteration-05 + agent-surface-audit `<!--` warning). |
| **Rules** | 8 native, all generic (portability hook excludes nothing under `.claude/rules/`). |
| **Hooks** | 5 enforcement hooks, each gated by `BLUEPRINT_HOOK_PROFILE`. |
