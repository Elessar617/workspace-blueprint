...
### 5.3 Routing & registry (2)

```
.claude/routing/refactor.md                  ← +architecture-audit entry
.claude/registry/harness-skills.json         ← 5 new skill entries
```
...
### 7.2 `.claude/registry/harness-skills.json`

Add five new entries with the same shape as existing native skill registrations. The `scripts/rebuild-registry.mjs` validator must pass with no errors after the edit.
...
- [ ] `.claude/registry/harness-skills.json` registers the 5 new skill entries.
